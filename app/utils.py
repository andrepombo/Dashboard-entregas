...
def set_pagination(request, items, item_numer):
    if not items:
    ...
    context = dict(items=items, page_range=page_range, last=pages_number, url_params=urlencode(url_params),item_len=item_len, item_numer=item_numer)
...
def set_me(request, items, item_numer):
    return items, {'current_page': page, 'items': item_len, 'items_on_page': item_numer}
